...
def render_acciones(r: Rendicion, rol: str):
        ...
        if st.button("Reenviar rendición corregida", key=f"reenviar_{rid}", type="primary"):
            if nuevo_archivo:
                if r["intentos_correccion"] >= INTENTOS_CORRECCION_MAX:
                    ...
                    r["archivo"] = nuevo_archivo.name
                    r["intentos_correccion"] += 1
                    r["observaciones"] = []
                    add_historial(rid, "Borrador", "Francisco Benavides", f"Comprobante corregido subido: {nuevo_archivo.name}")
                    add_historial(rid, "Pendiente", "Sistema", "Rendición reenviada. Aprobación previa de Supervisora queda nula. Re-notificada Catalina Vergara.")
                    add_notif("pendiente", f"{rid}: Rendición corregida. Supervisora debe re-aprobar.")
                    st.success("Rendición reenviada. Supervisora debe re-aprobar (ajuste #2 del modelo).")
                    st.rerun()
            else:
    ...
    if rol == "rendidor" and estado in ("Borrador", "Pendiente", "Observado"):
        if st.button("Cancelar esta rendición", key=f"cancel_{rid}"):
            add_historial(rid, "Cancelado", "Francisco Benavides", "Cancelación voluntaria por el rendidor.")
    ...
    if rol == "supervisora" and estado == "Pendiente":
        ...
        with col_a:
            if st.button("Aprobar pertinencia operativa", key=f"apr_{rid}", type="primary", use_container_width=True):
                if r["monto"] > MONTO_UMBRAL_GERENCIA:
                    add_historial(rid, "Aprobado", "Catalina Vergara", "Pertinencia operativa confirmada.")
        ...
        with col_b:
            if st.button("Rechazar", key=f"rec_{rid}", use_container_width=True):
                ...
                add_historial(rid, "Rechazado", "Catalina Vergara", nota)
    ...
    if rol == "finanzas" and estado == "Aprobado":
        ...
        with col_a:
            if st.button("Autorizar", key=f"aut_{rid}", type="primary", use_container_width=True):
                add_historial(rid, "Autorizado", "Mario Leal", "Documento válido en SII. Monto coincide.")
        ...
        with col_b:
            if st.button("Observar", key=f"obs_{rid}", use_container_width=True):
                ...
                if r["intentos_correccion"] >= INTENTOS_CORRECCION_MAX:
                    ...
                    add_historial(rid, "Observado", "Mario Leal", nota)
                    r["observaciones"].append(nota)
        ...
        with col_c:
            if st.button("Rechazar", key=f"rec_fin_{rid}", use_container_width=True):
                ...
                add_historial(rid, "Rechazado", "Mario Leal", nota)
                r["observaciones"].append(nota)
    ...
    if rol == "gerencia" and estado == "Pend. Gerencia":
        ...
        with col_a:
            if st.button("Aprobar gasto extraordinario", key=f"ger_apr_{rid}", type="primary", use_container_width=True):
                add_historial(rid, "Autorizado", "Gerencia de Finanzas", obs_g or "Gasto extraordinario aprobado por Gerencia.")
        ...
        with col_b:
            if st.button("Rechazar", key=f"ger_rec_{rid}", use_container_width=True):
                ...
                add_historial(rid, "Rechazado", "Gerencia de Finanzas", nota)
    ...
    if rol == "tesorera" and estado == "Autorizado":
        ...
        if st.button("Confirmar", key=f"pagar_{rid}", type="primary"):
            if "Sí" in hay_liquidez:
                add_historial(rid, "Pagado", "Rosa Pinto", "Transferencia ejecutada. Comprobante bancario subido.")
                ...
                add_historial(rid, "En Cola de Pago", "Rosa Pinto", "Sin liquidez en caja. Rendición en cola. Fecha estimada: 3 días hábiles (BR-10).")
    ...
    if rol == "tesorera" and estado == "En Cola de Pago":
        ...
        if st.button("Liberar pago manualmente (hay fondos)", key=f"liberar_{rid}", type="primary"):
            add_historial(rid, "Pagado", "Rosa Pinto", "Pago liberado manualmente desde lista de pagos pendientes.")
            add_historial(rid, "Finalizado", "Sistema", "Proceso cerrado. Archivado en solo lectura.")
